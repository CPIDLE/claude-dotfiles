...
2. **前景執行 opencode**：用 Bash 工具，透過 **pipe** 傳入 spec（避免 CLI 參數長度限制和 escape 問題）。**必須加 `--format json`**（default 格式走 TUI 無 stdout）：
   ```bash
   echo '<指令前綴 + spec 全文>' | opencode run --format json 2>&1
   ```
   指令前綴固定為：`你必須直接執行，不要規劃、不要問確認、不要等待許可。按照以下 spec 完成任務，完成後印出完成報告（列出新增/修改的檔案和測試結果）。`

   - 印出 `⚡ 執行中（<任務摘要>）`
   - 前景等待完成（pipe 模式不支援 background，但能拿到完整 stdout）
   - 設定 `timeout: 180000`（3 分鐘）
   - 前提：opencode.json 的 `permission.bash` 需設為 `"*": "allow"`（run 模式下 `ask` 會 auto-reject）

3. **執行完成後**：
   - **exit 0** → 繼續步驟 4
   - **exit 非零** → 印錯誤摘要（stdout 最後 10 行）並停止
   - **失敗偵測**：若 stdout 無任何 `tool_use` 事件 → 視為卡住，自動重跑一次
...
5. **背景執行 opencode**：用 Bash 工具（`run_in_background: true`）：
   ```bash
   opencode run --format json "你必須直接執行，不要規劃、不要問確認、不要等待許可。讀取 .opencode-agents/agent-<id>/spec.md 並立即按照 spec 完成任務。完成後印出完成報告，列出新增/修改的檔案和測試結果。" > .opencode-agents/agent-<id>/output.log 2>&1
   ```
