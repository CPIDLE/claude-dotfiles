...
2. **背景執行 opencode**：用 Bash 工具（`run_in_background: true`），將 spec 全文內嵌在 prompt。**必須加 `--format json`**（default 格式走 TUI 無 stdout）：
   ```bash
   opencode run --format json "你必須直接執行，不要規劃、不要問確認、不要等待許可。按照以下 spec 完成任務，完成後印出完成報告（列出新增/修改的檔案和測試結果）。
