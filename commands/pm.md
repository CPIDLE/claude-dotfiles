...
📌 <下次預計 1-2 項，逗號分隔>
⚠️ <重要記事，無則省略此行>
🕐 <YYYY-MM-DD HH:MM> 🔗 <Doc URL>
```

> ⚠️ Dashboard 是總覽，不是詳細記錄。每項用最短的詞描述，不寫完整句子。
> ⚠️ 使用標準 Unicode emoji（✅ 📌 ⚠️ 🔹 🔗 🕐），不使用平台特定語法。
...
- 每個專案只能有一個 `## 🔹` 區段，限 6 行以內
- 更新時以 `## 🔹 <專案名稱>` 定位段落，替換到下一個 `## 🔹` 或文件結尾
- 新增時在文件末尾 append 完整區段
- Dashboard 是總覽不是詳細記錄，用最短的詞描述，不寫完整句子
- 使用標準 Unicode emoji（✅ 📌 ⚠️ 🔹 🔗 🕐）
