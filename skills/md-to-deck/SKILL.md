...
name: md-to-deck
description: >
  Generate GYRO Systems branded single-file HTML slide deck directly from a Markdown source,
  then print to PDF via puppeteer-core + system Chrome. Use this skill whenever the user mentions
  GYRO Systems, GYRO 簡報, GYRO presentation, GYRO report, slide deck, 鑫蒂斯, GYRO 投影片,
  GYRO brand colors (#BD442C), or any GYRO-related slide deliverable. Output: single-file .html
  with fullscreen slides, keyboard/touch nav, progress bar, print support, Mermaid diagrams,
  full GYRO brand styling, plus optional verification.xlsx for calculations.
  For long-form A4 whitepapers use md-to-paper. For .pptx output use md-to-pptx.
  Trigger when: GYRO 簡報, GYRO slide deck, MD 轉投影片, 鑫蒂斯簡報, md-to-deck, /md-to-deck.
argument-hint: <source.md> [output.html]
---

# md-to-deck — MD → GYRO-branded HTML Slide Deck (→ PDF)
...
<style>
{CSS — copy from .claude/skills/md-to-deck/assets/gyro_css_template.css}
</style>
</head>
<body>
...
The full CSS is stored at `.claude/skills/md-to-deck/assets/gyro_css_template.css`. Read and embed it in every generated HTML `<style>` block. This CSS is extracted from the JS generator and must be used verbatim.
...
# Generate Excel
python .claude/skills/md-to-deck/scripts/gen_verification.py params.json verification.xlsx
```
...
```bash
node .claude/skills/md-to-deck/scripts/gyro_html_generator.js <content.json> <output.html>
```
See `.claude/skills/md-to-deck/assets/content_sample.json` for JSON schema reference.
...
| File | Purpose |
|------|---------|
| `.claude/skills/md-to-deck/assets/gyro_css_template.css` | **CSS template** — embed verbatim in every HTML (includes print CSS fix) |
| `.claude/skills/md-to-deck/assets/gyro_style_template.json` | Brand design system spec (reference) |
| `.claude/skills/md-to-deck/scripts/gyro_html_generator.js` | Legacy JSON→HTML generator |
| `.claude/skills/md-to-deck/assets/content_sample.json` | Legacy JSON schema example |
| `.claude/skills/md-to-deck/scripts/gen_verification.py` | Excel 驗算檔產生腳本 (需 openpyxl) |
